...
Говоришь или пишешь — бот распознаёт, категоризирует и сохраняет транзакции в Google Sheets. Yandex SpeechKit для распознавания речи, YandexGPT для AI-анализа, автоматические отчёты с графиками.

```
Голос/Текст → Yandex SpeechKit → YandexGPT → Подтверждение → Google Sheets
```
...
**Core:** python-telegram-bot • Yandex SpeechKit • YandexGPT • Google Sheets API • matplotlib • APScheduler
...
### 📊 Аналитика
- Сводный график за текущий месяц (доходы/расходы + donut по категориям)
- Годовые графики доходов и расходов по месяцам с линией среднего
- AI-отчёты по периодам (2 нед / 1 мес / 3 мес / 6 мес / 1 год)
...
├── services/
│   ├── speech.py      # Yandex SpeechKit STT
...
└── utils/
    ├── audio.py       # FFmpeg конвертация OGG→PCM
...
<b>Приватность</b>
<br>
<sub>Данные только в вашем<br>Google Sheets,<br>ALLOWED_USER_IDS защита</sub>
</td>
<td align="center" width="33%">
...
### 📊 Мониторинг
- Реальные метрики запросов
- Response time tracking (P50/P95/P99)
- External API health checks
- Auto rate limiting при деградации
...
**Мониторинг:**
- Кнопка "🔧 Состояние бота" в главном меню — полная статистика с реальными данными
